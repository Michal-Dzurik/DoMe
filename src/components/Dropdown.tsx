...
interface Props {
  ...
  items: DropdownItem[];
  onCreateRecordPress?: (name: string,selected: (selected: boolean) => void) => void;
      ...
      <View style={styles.searchContainer}>
        <TextInput style={styles.searchBox} placeholder={'Search'} onChangeText={(text) => {
          ...
          let filtered = props.items.filter(
            (item: DropdownItem) => {
              let include = item.name.toLowerCase().includes(searchWord.toLowerCase());
        ...
        }} value={searchWord} editable={!disabled} />
        ...
        {!disabled && props.items.length > 0 ?
          (
            filteredItems.map((item: DropdownItem) => (
              <TouchableOpacity style={styles.option} key={item.id} onPress={
                () => {
                  setDisabled(true);
